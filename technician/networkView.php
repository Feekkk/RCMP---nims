...
if ($assetId <= 0) { header('Location: network.php'); exit; }
...
$stmtWH = $pdo->prepare("SELECT warranty_id, warranty_start_date, warranty_end_date, warranty_remarks, created_at FROM warranty WHERE asset_type = 'network' AND asset_id = ? ORDER BY created_at DESC, warranty_id DESC");
...
$stmtC = $pdo->prepare("SELECT claim_id, claim_date, claim_time, issue_summary, claim_remarks, created_at FROM warranty_claim WHERE asset_type = 'network' AND asset_id = ? ORDER BY created_at DESC, claim_id DESC");
...
$stmtR = $pdo->prepare("SELECT repair_id, repair_date, completed_date, issue_summary, repair_remarks, created_at FROM repair WHERE asset_type = 'network' AND asset_id = ? ORDER BY created_at DESC, repair_id DESC");
...
$status_meta = [
    9 => ['icon'=>'ri-wifi-line','cls'=>'s-active', 'colour'=>'#10b981','bg'=>'rgba(16,185,129,0.1)', 'border'=>'rgba(16,185,129,0.25)'],
    10 => ['icon'=>'ri-wifi-off-line','cls'=>'s-muted',  'colour'=>'#64748b','bg'=>'rgba(100,116,139,0.1)','border'=>'rgba(100,116,139,0.25)'],
...
if ($deviceName === '') $deviceName = 'Network device';
...
<head>
    ...
    <title>Network #<?= (int)$assetId ?> — <?= htmlspecialchars($deviceName) ?> — RCMP NIMS</title>
...
<main class="main-content">
    ...
    <header class="page-header">
        <div class="page-header-left">
            <a class="back-link" href="network.php"><i class="ri-arrow-left-line"></i> Back to inventory</a>
            <h1 class="page-title">
                <?= htmlspecialchars($deviceName) ?>
                <span class="asset-id-badge">#<?= (int)$assetId ?></span>
            </h1>
            <p class="page-subtitle">Network asset details and activity history.</p>
        </div>
        <div class="header-actions">
            <?php if ($statusId === 9): ?>
                <a class="btn btn-action" href="networkDeploy.php?asset_id=<?= urlencode((string)$assetId) ?>"><i class="ri-upload-2-line"></i> Deploy</a>
            <?php elseif ($statusId === 3): ?>
                <a class="btn btn-action" href="networkReturn.php?asset_id=<?= urlencode((string)$assetId) ?>"><i class="ri-inbox-unarchive-line"></i> Return</a>
            <?php endif; ?>
        </div>
    </header>
            ...
            <div class="device-hero">
                ...
                <div>
                    ...
                    <div class="device-sub">
                        ...
                        <span class="device-sub-item"><i class="ri-global-line"></i> <span class="mono-val"><?= htmlspecialchars((string)($asset['ip_address'] ?? '—')) ?></span></span>
                        <span class="device-sub-item"><i class="ri-links-line"></i> <span class="mono-val"><?= htmlspecialchars((string)($asset['mac_address'] ?? '—')) ?></span></span>
                    </div>
                </div>
            ...
            <div class="tab-panel active" id="tab-specs">
                <div class="kv-grid">
                    ...
                    <div class="kv-row">
                        ...
                        <span class="kv-val"><?= htmlspecialchars((string)($asset['ip_address'] ?? '—')) ?></span>
                    </div>
                    <div class="kv-row">
                        <span class="kv-label">MAC address</span>
                        <span class="kv-val"><?= htmlspecialchars((string)($asset['mac_address'] ?? '—')) ?></span>
                    </div>
