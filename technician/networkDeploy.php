...
const NETWORK_ALLOWED_DEPLOY_FROM = [9];
...
try {
    ...
    $assetOptions = [];
    $error_message = 'Could not load network assets. Ensure the database is available.';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error_message === '') {
    ...
    if ($assetId <= 0) {
        ...
        try {
            ...
            if (!in_array($currentStatus, NETWORK_ALLOWED_DEPLOY_FROM, true)) {
                throw new RuntimeException('Only Online (9) assets can be deployed.');
            }
            ...
            $upd = $pdo->prepare('UPDATE network SET status_id = :deploy WHERE asset_id = :asset_id');
            $upd->execute([
...
function network_label(array $row): string
{
    $parts = [];
    ...
    if (!empty($row['ip_address'])) $parts[] = 'IP ' . (string)$row['ip_address'];
...
<head>
    ...
    <title>Deploy Network Asset — RCMP NIMS</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .icon{width:36px;height:36px;border-radius:10px;background:rgba(14,165,233,.12);display:flex;align-items:center;justify-content:center;flex-shrink:0}
        .icon i{color:#0ea5e9;font-size:1rem}
...
<main class="main-content">
    <header class="page-header">
        <div>
            <h1 class="page-title">Deploy Network Asset</h1>
            ...
            <div class="card-head">
                <div class="head-left">
                    ...
                    <div>
                        ...
                        <div class="head-desc">Only Online (9) can be deployed.</div>
                    </div>
                </div>
            </div>
            <div class="card-body">
                        ...
                        <select name="asset_id" class="select" required>
                            ...
                            <?php foreach ($assetOptions as $opt): $aid = (int)$opt['asset_id']; ?>
                                <option value="<?= $aid ?>" <?= $aid === $assetId ? 'selected' : '' ?>>
                                    <?= htmlspecialchars(network_label($opt)) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <?php if ($asset): ?>
            ...
            <div class="card-head">
                <div class="head-left">
                    ...
                    <div>
                        ...
                        <div class="head-desc">Saved into <code>network_deployment</code>.</div>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <div class="actions">
                    <a class="btn btn-outline" href="networkDeploy.php"><i class="ri-refresh-line"></i> Clear</a>
