...
try {
    $statusOptions = $pdo->query("
        ...
        FROM status
        WHERE status_id IN (1,2,5,6,7,8)
        ORDER BY status_id
...
if ($assetId > 0) {
    ...
    if (!$asset) {
        ...
        $chk = $pdo->prepare('SELECT 1 FROM av_return WHERE deployment_id = ? LIMIT 1');
        $chk->execute([(int)$latestDeployment['deployment_id']]);
        $returnAlreadyRecorded = (bool)$chk->fetchColumn();
        if ($returnAlreadyRecorded) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error_message === '') {
    ...
    if ($assetId <= 0) {
        ...
        try {
            ...
            $dup = $pdo->prepare('SELECT 1 FROM av_return WHERE deployment_id = ? LIMIT 1');
            ...
            $upd = $pdo->prepare('UPDATE av SET status_id = :status_id WHERE asset_id = :asset_id');
            $upd->execute([
                ':status_id' => $newStatusId > 0 ? $newStatusId : DEFAULT_RETURN_STATUS_ID,
                ':asset_id' => $assetId,
            ]);
...
function av_name(array $a): string
{
    $t = trim(($a['category'] ?? '') . ' ' . ($a['brand'] ?? '') . ' ' . ($a['model'] ?? ''));
    return $t !== '' ? $t : 'AV Asset';
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Return AV Asset — RCMP NIMS</title>
...
<main class="main-content">
    <header class="page-header">
        <div>
            <h1 class="page-title">Return AV Asset</h1>
            <div class="page-subtitle">Record a return for the latest AV deployment.</div>
        </div>
        ...
        <div class="card">
            <div class="card-head">
                <div class="head-left">
                    ...
                    <div>
                        <div class="head-title"><?= htmlspecialchars(av_name($asset)) ?></div>
    ...
    <?php if ($error_message === '' && $asset && $latestDeployment): ?>
                ...
                <div class="card-head">
                    <div class="head-left">
                        ...
                        <div>
                            ...
                            <div class="head-desc">Saved into <code>av_return</code> and updates AV status.</div>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <div class="field col-2">
                            ...
                            <select name="new_status_id" class="select">
                                <?php
                                    ...
                                    if ($statusOptions === []) {
                                        echo '<option value="' . DEFAULT_RETURN_STATUS_ID . '" selected>Active (1)</option>';
                                    } else {
                    ...
                    <div class="actions">
                        <a class="btn btn-outline" href="avReturn.php?asset_id=<?= (int)$assetId ?>"><i class="ri-refresh-line"></i> Reset</a>
